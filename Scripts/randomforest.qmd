...
for (i in 1:10) {
          ...
          #split the data
          data_train <- maree_summary %>% sample_frac(0.75)  
...
rf.cutoff = randomForest(Bycatch~.,data=maree_summary,cutoff=c(1-rare.class.prevalence,rare.class.prevalence), ntree = 1000, do.trace = 100)
print(rf.cutoff)


#2 - Balancing by sampling stratification ////// THIS TAKES FOREVER TO RUN? ///////
nRareSamples = 10 * rare.class.prevalence
rf.strata = randomForest(Bycatch~.,data=data_train,strata=data_train$Bycatch,
                         sampsize=c(nRareSamples,nRareSamples))
print(rf.strata)


#3 - Balancing by class-weight during training.
rf.classwt = randomForest(Bycatch~.,data=data_train,classwt=c(0.01,100))
print(rf.classwt)
...
#### try regularized random forest RRF

rrf <- RRF(Bycatch ~. , data = maree_summary, ntree = 10000, do.trace = 100, coefReg = 0.95, sampsize = c(15,10), mtry = 20)
rrf
