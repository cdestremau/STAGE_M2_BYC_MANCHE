...
#sS__flottille pr marsouins : Chalutiers de fond exclusifs, Fileyeurs exclusifs
...
sp_p_w <- pelagique_ptmbss %>%
  ...
  reframe(
    ...
    Hareng_weight = sum(if_else(SPP_COD_FAO == "Clupea harengus", POIDS_REF, 0),na.rm = T),
    #Hareng_p = sum(if_else(SPP_COD_FAO == "Clupea harengus", POIDS_REF, 0),na.rm = T) / Poids_tot_MAREE,
...
maree_summary <- maree_summary %>% select (-second_sp_fished, -third_sp_fished, -second_sp_w, -third_sp_w, -ID_MAREE, -Trachurus_weight, -Gadidae_weight, -Scomber_weight, -Merluccius_weight, -Dicentrarchus_weight, -Sprat_weight, -Anchois_weight, -Sardine_weight, -Hareng_weight)
...
#maree_summary1 <- maree_summary %>% select(-Bycatch)

famd_byc <- FAMD(maree_summary[,-13], graph = T, ncp = 25)
...
quanti.var 
famd_var <- fviz_famd_var(famd_byc, "quanti.var", repel = TRUE,
              col.var = "black", axes = c(1,2) , ggtitle(""))
...
ggsave("famd_var_plot.png", plot = famd_var, width = 5000, height = 5000, units = "px", bg = "white", dpi = 1000)
...
fviz_famd_var(famd_byc, "quanti.var", 
             repel = TRUE)
...
famd_quali_plot <- fviz_famd_var(famd_byc, "quali.var", col.var = "black")



ggsave("famd_quali_plot.png", plot = famd_quali_plot, width = 5500, height = 5000, units = "px", bg = "white", dpi = 600)


### graph of individuals 

fviz_famd_ind(famd_byc,
             repel = F)
famd_ind_plot <- fviz_famd_ind(famd_byc, repel = FALSE, geom = "point", col.ind = "black") + theme_minimal() + ggtitle ("")

ggsave("famd_ind_plot", plot = famd_ind_plot, width = 4000, height = 4000, units = "px", bg = "white", dpi = 600)
...
#data_byc <- maree_summary %>% select (-Bycatch, -first_fam_fished)
#data_byc <- data.frame(data_byc)
...
res.hcpc <- HCPC(famd_byc,consol = T, method = "complete", metric = "euclidian", nb.clust = 12)
...
## Plot
cluster <- fviz_cluster(res.hcpc, geom = "point", axes = c(1,2)) +ggtitle(label='') +
  theme(
    axis.title.x = element_text(size = 14),
    axis.title.y = element_text(size = 14)
  )

clust1 <- as.data.frame(res.hcpc$desc.var$quanti$"1")
clust1_qual <- as.data.frame(res.hcpc$desc.var$category$"1")



clust2 <-  as.data.frame(res.hcpc$desc.var$quanti$"2")
clust2_qual <- as.data.frame(res.hcpc$desc.var$category$"2")


clust1_qual[] <- lapply(clust1_qual, function(x) if(is.numeric(x)) round(x, 2) else x)

clust2_qual[] <- lapply(clust2_qual, function(x) if(is.numeric(x)) round(x, 2) else x)


clust1[] <- lapply(clust1, function(x) if(is.numeric(x)) round(x, 2) else x)

clust2[] <- lapply(clust2, function(x) if(is.numeric(x)) round(x, 2) else x)


names(pipo)[which(names(pipo) == "maree_summary$Bycatch")] <- "Bycatch"


ggsave("cluster.png", plot = cluster, width = 5000, height = 6000, units = "px", bg = "white", dpi = 600)



#maree_summary[, c(-21, -26)]
...
val_df <- as.data.frame(res.pca$ind)
...
longueur <- partialPlot(rf.strata, as.data.frame(maree_summary), which.class = 1, Longueur, cex.name = 0.9)

puissance <- partialPlot(rf.strata, as.data.frame(maree_summary), which.class = 1, av_puissance, cex.name = 0.9)
  
vitesse <- partialPlot(rf.strata, as.data.frame(maree_summary), which.class = 1, av_vitesse, cex.name = 0.9)

fam <- partialPlot(rf.strata, as.data.frame(maree_summary), which.class = 1, fam_plus_peche, cex.name = 0.9)

gadide <- partialPlot(rf.strata, as.data.frame(maree_summary), which.class = 1, Gadidae_p, cex.name = 0.9)

bar <- partialPlot(rf.strata, as.data.frame(maree_summary), which.class = 1, Dicentrarchus_p, cex.name = 0.9)
...
rf3 <- randomForest(Bycatch ~., data = maree_rf, ntree = 3000, do.trace = 100)
rf3
...
maree_rf <- pipo %>% filter (clust %in% c(1,2)) %>% select (-clust)

#maree_rf$Bycatch <- maree_rf$`maree_summary$Bycatch`

maree_rf %>% select (-maree_summary$Bycatch)


maree_rf2 <- maree_rf %>% select (Dicentrarchus_p, av_mail, av_peche, av_vitesse, Gadidae_p, av_puissance, Trachurus_p, n_especes, Scomber_p, poids_tot, poids_total_sp_plus_peche, Bycatch, fam_plus_peche)

## for the random forest, i keep clust 1 and 2, but do i also keep the variables that were most significant?


rf.strata = randomForest(Bycatch~.,data=maree_summary, sampsize=c(22,11), proximity = T, keep.forest = T, importance = T, ntree = 1000, do.trace = 100, mtry = 2)

print(rf.strata)

varImpPlot(rf.strata)

conf_matrix <- as.data.frame(rf.strata$confusion)

imp <- importance(rf.strata)

impvar <- rownames(imp)[order(imp[, 1], decreasing=TRUE)]
op <- par(mfrow=c(2, 3))
for (i in seq_along(impvar)) {
    partialPlot(rf.strata, maree_summary, impvar[i], xlab=impvar[i],
...
importance_values <- importance(rf.strata, type = 1)  # Type 1 corresponds to Mean Decrease in Accuracy
...
ggsave("var_imp-plot.png", plot = var_imp_plot, width = 5000, height = 6000, units = "px", bg = "white", dpi = 600)
